...
str int_operand_to_string(arena* a, int_inst::operand op, int_inst_spec::operand ty) {
	switch(ty) {
		case int_inst_spec::REG: return str_cstring(int_reg_name((u32)op.reg));
		case int_inst_spec::IMM: return str_push_fmt(a, "%lld", (i64)op.i);
...
void int_print_enabled_extensions(u32 mask) {
	...
	for(u32 i = 0; i < INT_EXT_COUNT; ++i) {
		if(mask & INT_EXT_BITS[i]) {
			printf("%s%s", first ? "" : ", ", INT_EXT_NAMES[i]);
			first = false;
		}
	}
}
