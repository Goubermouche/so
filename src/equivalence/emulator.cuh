...
namespace so {
	...
	__host__ __device__ __forceinline__ void execute_inst(cpu_state& state, const inst& inst) {
		u32& dst = state.regs[inst.ops[0]];
		switch(inst.tag) {
			case INST_MOV_RR: dst = state.regs[inst.ops[1]];       break;
			case INST_SUB_RI: dst = dst - inst.ops[1];             break;
			case INST_SUB_RR: dst = dst - state.regs[inst.ops[1]]; break;
			case INST_NOT_R:  dst = ~dst;                          break;
			case INST_AND_RR: dst = dst & state.regs[inst.ops[1]]; break;
