...
SO_HD bool ext_rv64i_run(u32 op, u64 regs[32], const inst& in) {
	...
	switch(op) {
	case OP_ADDIW:
		ext_rv_wr(regs, d, (u64)ext_rv_sext32((u32)regs[in.operands[1].reg] + (u32)in.operands[2].i));
		return true;
	case OP_SLLIW:
		ext_rv_wr(regs, d,
	...
	case OP_SRLIW:
		...
		return true;
	case OP_SRAIW:
		ext_rv_wr(regs, d, (u64)((i64)((i32)regs[in.operands[1].reg] >> (in.operands[2].i & 0x1F))));
		return true;
	case OP_ADDW:
		ext_rv_wr(regs, d,
