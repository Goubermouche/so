...
inline bool ext_rv64m_smt(Z3_context ctx, smt_state* s, u32 op, u32 d, u32 s1, u32 s2,
													Z3_ast /*imm*/) {
	switch(op) {
	case OP_MULW: {
		Z3_ast a = Z3_mk_extract(ctx, 31, 0, s->r[s1]);
		Z3_ast b = Z3_mk_extract(ctx, 31, 0, s->r[s2]);
		smt_wr(ctx, s, d, Z3_mk_sign_ext(ctx, 32, Z3_mk_bvmul(ctx, a, b)));
		return true;
	}
	case OP_DIVW: {
		Z3_ast a = Z3_mk_extract(ctx, 31, 0, s->r[s1]);
		...
		smt_wr(ctx, s, d, Z3_mk_sign_ext(ctx, 32, res));
		return true;
	}
	case OP_DIVUW: {
		Z3_ast a = Z3_mk_extract(ctx, 31, 0, s->r[s1]);
