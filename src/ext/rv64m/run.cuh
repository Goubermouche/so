...
namespace sup {
	SO_HD bool ext_rv64m_run(u32 op, u64 regs[32], const inst& in) {
		...
		switch(op) {
			...
			case OP_DIVUW: {
				const u32 ua = (u32)a;
				const u32 ub = (u32)b;
				rv_wr(regs, d, (u64)rv_sext32(ub == 0 ? (u32)-1 : ua / ub));
				return true;
			}
			case OP_REMW: {
				const i32 sa = (i32)a;
			...
			case OP_REMUW: {
				const u32 ua = (u32)a;
				const u32 ub = (u32)b;
				rv_wr(regs, d, (u64)rv_sext32(ub == 0 ? ua : ua % ub));
				return true;
			}
		}
