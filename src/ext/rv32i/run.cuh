...
SO_HD bool ext_rv32i_run(u32 op, u64 regs[32], const inst& in) {
	...
	switch(op) {
	case OP_ADD: ext_rv_wr(regs, d, regs[in.operands[1].reg] + regs[in.operands[2].reg]); return true;
	case OP_SUB: ext_rv_wr(regs, d, regs[in.operands[1].reg] - regs[in.operands[2].reg]); return true;
	case OP_SLL:
	...
	case OP_SLTU:
		...
		return true;
	case OP_XOR: ext_rv_wr(regs, d, regs[in.operands[1].reg] ^ regs[in.operands[2].reg]); return true;
	case OP_SRL:
		ext_rv_wr(regs, d, regs[in.operands[1].reg] >> (regs[in.operands[2].reg] & 0x3F));
		return true;
	case OP_SRA:
		ext_rv_wr(regs, d, (u64)((i64)regs[in.operands[1].reg] >> (regs[in.operands[2].reg] & 0x3F)));
		return true;
	case OP_OR: ext_rv_wr(regs, d, regs[in.operands[1].reg] | regs[in.operands[2].reg]); return true;
	case OP_AND: ext_rv_wr(regs, d, regs[in.operands[1].reg] & regs[in.operands[2].reg]); return true;
